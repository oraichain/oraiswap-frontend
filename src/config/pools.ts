...
// Combine information related to pair into Pairs class
export class Pairs {
  ...
  public static pairDenoms = uniq(flatten(this.pairs.map((pair) => pair.asset_denoms)));
  public static poolTokens = cosmosTokens.filter((token) => this.pairDenoms.includes(token.denom));
  static getPair = (denom1: string | string[], denom2?: string): Pair | undefined => {
