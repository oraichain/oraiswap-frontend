import { AIRI_BSC_CONTRACT, ORAI_BRIDGE_EVM_DENOM_PREFIX, ORAI_BSC_CONTRACT } from 'config/constants';
import { processWsResponseMsg, toAmount } from 'libs/utils';
...
describe('bigint', () => {
    ...
    it('websocket-message-packets-happy-path-should-return-correct-tokens', async () => {
        const msg = {
            result: {
                events: {
                    'recv_packet.packet_data': [`{"amount":"100000000000000","denom":"${ORAI_BRIDGE_EVM_DENOM_PREFIX}${ORAI_BSC_CONTRACT}","receiver":"orai...","sender":"oraib..."}`, `{"amount":"1000000000000000000","denom":"${ORAI_BRIDGE_EVM_DENOM_PREFIX}${AIRI_BSC_CONTRACT}","receiver":"orai...","sender":"oraib..."}`]
                }
            }
        };
        const result = processWsResponseMsg(msg);
