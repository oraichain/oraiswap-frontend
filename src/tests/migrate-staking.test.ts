...
export async function setupSimulateClient() {
  ...
  console.log(
    "Migrate Staking contract successfully at tx:",
    migrateResult.transactionHash,
  );

  const newStakingInstance = new OraiswapStakingClient(
    client,
    owner,
    STAKING_CONTRACT,
  );
...
describe("Migrate staking contract", () => {
  ...
  it.each(liquidAddrs)(
    `must have reward info for liquid address %s`,
    async (address) => {
  ...
  it("should return pools info correctly", async () => {
    const allRWPerSec = await fetchAllRewardPerSecInfos(liquidAddrs);
    const allTokenAssetPools = await fetchAllTokenAssetPools(liquidAddrs);
    console.log({ allRWPerSec, allTokenAssetPools });
    expect(allRWPerSec.length).toBe(liquidAddrs.length);
    expect(allTokenAssetPools.length).toBe(liquidAddrs.length);
  });
});
