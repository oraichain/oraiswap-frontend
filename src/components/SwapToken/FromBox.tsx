...
import { ButtonToggle } from 'components/layouts/Buttons';
import { CenterV } from 'components/layouts/Containers';
import { TokenInSwapConfig } from 'components/SwapToken/models';
import {
  TokenBoxContainer,
  TokenBoxRow
} from 'components/SwapToken/StyledTokenBox';
import { Text } from 'components/Texts';
import { useBooleanStateWithWindowEvent } from 'hooks/useBooleanStateWithWindowEvent';
import { TokenAmountInput } from 'pages/main/components/TokenAmountInput';
...
export const FromBox = observer(function FromBox({
  config,
  dropdownStyle,
  dropdownClassName,
  ...props
  ...
  const balance = queries.queryBalances
    ...
    .balances.find(
      (bal) =>
        bal.currency.coinMinimalDenom === config.sendCurrency.coinMinimalDenom
    );
      ...
      <TokenBoxRow>
        ...
        <CenterV>
          ...
          <MaxButton
            type="button"
            size="small"
            isActive={config.isMax}
            onClick={handleMaxButtonToggled}
          >
            <Text
              size="xs"
              emphasis="medium"
      ...
      <TokenBoxRow>
        <TokenSelect
          options={config.sendableCurrencies.filter(
            (cur) =>
              cur.coinMinimalDenom !== config.outCurrency.coinMinimalDenom
          )}
