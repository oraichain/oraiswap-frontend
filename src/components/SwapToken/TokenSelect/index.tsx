...
import { TokenSelectList } from 'components/SwapToken/TokenSelect/TokenSelectList';
import { Text } from 'components/Texts';
import { colorGold, colorTextIcon } from 'emotionStyles/colors';
...
export function TokenSelect({
  ...
  const isSingleToken =
    options.length === 1 && options[0].coinDenom === value.coinDenom;

  const extraAssetInfo = extraAssetInfos
    ? extraAssetInfos.find(
        (extraAssetInfo) => extraAssetInfo.coinDenom === value.coinDenom
      )
    : null;
  return (
    <TokenSelectContainer {...props}>
      {extraAssetInfo ? (
        <>
          ...
          >
            <span>
              {extraAssetInfo.liquidityWeightPercentage
                .maxDecimals(0)
                .toString()}
              %
            </span>
            <div className="slice">
              <div
                style={{
                  background: `${
                    borderImages[MISC.GRADIENTS[extraAssetInfo.index]]
                  }`
                }}
                className="bar"
              />
          ...
          <CenterV>
            <div>
              <h5 className="text-base md:text-xl text-white-high">
                {value?.coinDenom?.toUpperCase()}
              </h5>
              {channelShown && (
                <ChannelText isMobileView={isMobileView} currency={value} />
              )}
            </div>
      ...
      {isDropdownOpen && (
        <TokenSelectList
          style={{
            ...dropdownStyle,
            display: !isDropdownOpen ? 'none' : undefined
          }}
...
function ChannelText({
  ...
  return (
    <Text
      ...
      style={{ marginTop: 4, color: colorTextIcon }}
    >
      {channelId}
    </Text>
  );
