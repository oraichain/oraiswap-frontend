...
import { SubTitleText, Text } from 'components/Texts';
import {
  colorPrimary,
  colorPrimaryDark,
  colorPrimaryLight,
      ...
      <TokenItemList>
        {filteredCurrencies.map((cur) => {
          const balance = queries.queryBalances
            ...
            .balances.find(
              (bal) => bal.currency.coinMinimalDenom === cur.coinMinimalDenom
            );
          ...
          const extraAssetInfo = extraAssetInfos
            ? extraAssetInfos.find(
                (extraAssetInfo) => extraAssetInfo.coinDenom === cur.coinDenom
              )
            : undefined;
...
function TokenItem({
  ...
  return (
    <TokenItemContainer
      ...
      {...props}
      className={extraAssetInfo ? '!pr-5 md:!pr-15' : ''}
    >
      <CenterV>
        {extraAssetInfo ? (
          <figure
        ...
        <div className={`${extraAssetInfo ? '' : 'ml-3'} flex-shrink-0`}>
          <h5
            className={`text-base ${
              extraAssetInfo ? 'md:text-xl' : 'md:text-lg'
            } text-white-high`}
          >
            {currency.coinDenom.toUpperCase()}
          </h5>
          {'paths' in currency && currency.paths.length > 0 ? (
            <p className="text-white-mid text-xs md:text-base font-normal">
