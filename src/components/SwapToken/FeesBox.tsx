...
export const FeesBox = observer(function FeesBox({ config, ...props }: Props) {
  ...
  const swapFeeText = isPoolSwapConfig(config)
    ? `${config.swapFee.trim(true).maxDecimals(3).toString()}%`
    : config.swapFees
        .map((swapFee) => {
          return `${swapFee.trim(true).maxDecimals(3).toString()}%`;
        })
        .join(' + ');
    ...
    <FeeBoxContainer {...props}>
      ...
      <Section>
        ...
        <Text
          ...
          weight="semiBold"
          style={
            !isPoolSwapConfig(config) && config.showWarningOfSlippage
              ? { color: colorError }
              : undefined
          }
        >
          {new RatePretty(config.estimatedSlippage)
...
function isPoolSwapConfig(
  config: PoolSwapConfig | TradeSwapConfig
): config is PoolSwapConfig {
