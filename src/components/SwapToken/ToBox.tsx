...
import { TokenOutSwapConfig } from 'components/SwapToken/models';
import {
  TokenBoxContainer,
  TokenBoxRow
...
export const ToBox = observer(function ToBox({
  config,
  dropdownClassName,
  dropdownStyle,
  ...props
      ...
      <TokenBoxRow>
        <TokenSelect
          options={config.sendableCurrencies.filter(
            (cur) =>
              cur.coinMinimalDenom !== config.sendCurrency.coinMinimalDenom
          )}
          value={config.outCurrency}
          onSelect={(appCurrency) =>
            config.setOutCurrency(appCurrency.coinMinimalDenom)
          }
        ...
        <TitleText
          ...
          pb={0}
          style={{
            opacity: config.outAmount.toDec().equals(new Dec(0))
              ? 0.4
              : undefined,
            textAlign: 'right'
          }}
