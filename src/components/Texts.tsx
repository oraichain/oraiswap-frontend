...
function mapTextPropsToCssProps({
  ...
  const fontTypeProps = ['2xl', 'xl', 'lg'].some(
    (largeSize) => size === largeSize
  )
    ? {
        lineHeight: 1,
        fontFamily: `${fontType ?? 'Poppins'}, ui-sans-serif, system-ui`
      }
    : {
        lineHeight: undefined,
        fontFamily: `${fontType ?? 'Inter'}, ui-sans-serif, system-ui`
      };
  const textOpacity = solidColorList.some((solidColor) => solidColor === color)
    ? 1
    : OpacityByEmphasis[emphasis];
  return {
    ...fontTypeProps,
    fontSize: isMobileView
      ? FontSizeBySizeInMobileView[
          size as keyof typeof FontSizeBySizeInMobileView
        ]
