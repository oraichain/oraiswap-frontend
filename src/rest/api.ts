...
import { TokenSwap } from 'constants/pools';
...
async function fetchTokenInfo(tokenSwap: TokenSwap): TokenInfo {
  ...
  if (!tokenSwap.contractAddress) {
    ...
    const data = await querySmart(tokenSwap.contractAddress, { token_info: {} });
    tokenInfo = {
      ...tokenInfo,
      symbol: data.symbol,
      name: data.name,
...
async function fetchBalance(
  ...
  if (!tokenAddr) return fetchNativeTokenBalance(walletAddr, denom);
