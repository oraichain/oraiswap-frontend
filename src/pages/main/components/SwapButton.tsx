...
import { CtaButton } from 'components/layouts/Buttons';
import { Spinner } from 'components/Spinners';
import { Text } from 'components/Texts';
import { colorError } from 'emotionStyles/colors';
...
export const SwapButton = observer(function SwapButton({ config }: Props) {
  ...
  const isSwapPoolsFetching =
    currentSwapPools.find((poolId) => {
      return queries.osmosis.queryGammPools.getObservableQueryPool(poolId)
        .isFetching;
    }) != null;
  ...
  return (
    <CtaButton
      ...
      style={
        config.showWarningOfSlippage
          ? { backgroundColor: colorError }
          : undefined
      }
      disabled={
        !account.isReadyToSendMsgs ||
        config.poolIds.length === 0 ||
    ...
    >
      {account.isSendingMsg === 'swapExactAmountIn' ? (
        ...
        >
          {isSwapPoolsFetching && (
            <Spinner style={{ marginRight: 12, display: 'inline-block' }} />
          )}
