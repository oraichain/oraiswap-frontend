...
import { SlippageStep } from 'pages/main/models/tradeModels';
import { TradeConfig } from 'pages/main/stores/trade/config';
...
export const TradeTxSettings = observer(function TradeTxSettings({
  config
}: Props) {
    ...
    <TradeTxSettingsContainer>
      ...
      >
        ...
        <SlippageToleranceList>
          {[SlippageStep.Step1, SlippageStep.Step2, SlippageStep.Step3].map(
            (slippageStep) => {
              return (
...
const SlippageToleranceInputItem = observer(
  function SlippageToleranceInputItem({ config }: Props) {
      ...
      <SlippageToleranceInputItemContainer selected={selected} error={error}>
        ...
        >
          <AutosizeInput
            ...
            minWidth={
              config.manualSlippageText === config.initialManualSlippage
                ? 24
                : undefined
            }
...
const SlippageToleranceInputItemContainer = styled.li<{
  selected: boolean;
  error?: Error;
}>`
  display: flex;
  ...
  ${({ selected, error }) => {
    ...
    return selected
      ? { backgroundColor: colorPrimary200 }
