...
import { FeesBox } from 'components/SwapToken/FeesBox';
import { FromBox } from 'components/SwapToken/FromBox';
import { ToBox } from 'components/SwapToken/ToBox';
import { colorPrimary, colorPrimaryLight } from 'emotionStyles/colors';
import { cssRaiseButtonShadow } from 'emotionStyles/forms';
import { cssAbsoluteCenter } from 'emotionStyles/layout';
import { useFakeFeeConfig } from 'hooks/tx';
import { Clip } from 'pages/main/components/TradeClipboard/Clip';
...
import { SwapDirectionButton } from 'components/SwapToken/SwapDirectionButton';
...
export const TradeClipboard: FunctionComponent = observer(() => {
  ...
  useEffect(() => {
    if (query.from) {
      const currency = config.sendableCurrencies.find(
        (cur) => cur.coinDenom === query.from
      );
  ...
  useEffect(() => {
    for (const currency of config.sendableCurrencies) {
      ...
      chainStore
        ...
        .findCurrency(currency.coinMinimalDenom);
    }
    ...
    <React.Fragment>
      <TradeClipboardContainer>
        ...
        <TradeClipboardContent
          style={isMobileView ? { maxHeight: '524px' } : undefined}
        >
