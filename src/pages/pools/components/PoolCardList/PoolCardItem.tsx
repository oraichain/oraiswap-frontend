...
import { applyOptionalDecimal } from 'libs/format';
import { Optional } from 'utility-types';
import { CardInfoPlaceholder } from 'components/common/CardInfoPlaceholder';

export const PoolCardItem = function PoolCardItem(
  props: (
    | IncentivizedPoolCardProp
    | Optional<MyPoolCardProp, 'myLiquidity'>
  ) & {
  ...
  const isSuperfluidEnabled =
    queries.osmosis.querySuperfluidPools.isSuperfluidPool(poolId);

  return (
    <div
      className={`rounded-xl p-0.25 hover:bg-enabledGold ${
        isSuperfluidEnabled ? 'bg-sfs hover:bg-none' : 'bg-card'
      }`}
      onClick={onClick}
    >
