...
import { ButtonFaint } from 'components/layouts/Buttons';
import { Text, TitleText } from 'components/Texts';
import { cssAnimateSpin } from 'emotionStyles/animations';
import { colorGold } from 'emotionStyles/colors';
import { LinkIcon } from 'pages/assets/components/LinkIcon';
import {
  TableData,
  TableHeaderRow,
  TableRow
} from 'pages/assets/components/Table';
import { useStore } from 'stores';
import {
  IBCTransferHistory,
  IBCTransferHistoryStatus,
  UncommitedHistory
} from '../../stores/ibc-history';
import useWindowSize from 'hooks/useWindowSize';

const tableWidths = ['20%', '15%', '15%', '30%', '20%'];
export const IbcTransferHistoryList = observer(
  function IbcTransferHistoryList() {
    ...
    return (
      <>
        <div className="px-5 md:px-0">
          <TitleText
            style={{ paddingTop: isMobileView ? 16 : 40 }}
...
const IbcTransferHistoryRow = observer(function IbcTransferHistoryRow({
  history
}: IbcTransferHistoryRowProps) {
    ...
    <TableRow>
        ...
        {chainStore.getChain(history.sourceChainId).raw.explorerUrlToTx ? (
          <a
            href={chainStore
              ...
              .raw.explorerUrlToTx.replace(
                '{txHash}',
                history.txHash.toUpperCase()
              )}
            target="_blank"
      ...
      <TableData style={{ width: tableWidths[1] }}>
        ...
        {'sequence' in history ? (
          ...
          <SpinningImg alt="loading" src="/assets/Icons/Loading.png" />
        )}
      </TableData>
      ...
      <TableData align="right" style={{ width: tableWidths[4] }}>
        <Text emphasis="high" style={{ display: 'flex', alignItems: 'center' }}>
          <StatusTableData
            status={'status' in history ? history.status : undefined}
          />
        </Text>
      </TableData>
    </TableRow>
...
function StatusTableData({ status }: { status?: IBCTransferHistoryStatus }) {
  ...
  switch (status) {
    case 'complete':
      return (
        <>
          <StatusImg alt="success" src="/assets/Icons/ToastSuccess.png" />
          Success
        </>
      );
    case 'pending':
    ...
    case 'refunded':
      return (
        <>
          <StatusImg alt="failed" src="/assets/Icons/FailedTx.png" />
          Refunded
        </>
      );
    case 'timeout':
...
const PaginationButton = styled(ButtonFaint)<{ selected?: boolean }>`
  ...
  padding-right: 12px;
  ${({ selected = false }) =>
    selected ? { border: `1px solid ${colorGold}` } : null}
`;
