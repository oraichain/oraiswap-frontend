...
export const AssetsOverview: FunctionComponent<{ title: string }> = observer(
  ({ title }) => {
    ...
    const lockedCoins = queries.osmosis.queryLockedCoins.get(
    ...
    const unbondingBanace =
      queries.cosmos.queryUnbondingDelegations.getQueryBech32Address(
        account.bech32Address
      ).total;
    const delegatedBalancePrice = calcTotalFiatValue([
      delegatedBalance,
      unbondingBanace
    ]);

    function calcTotalFiatValue(balanceList: CoinPretty[]) {
      let fiatValue = new PricePretty(
      ...
      for (const balance of balanceList) {
        if (balance.currency.coinMinimalDenom.startsWith('gamm/pool/')) {
          ...
          if (pool) {
            ...
            if (
              tvl.toDec().gt(new Dec(0)) &&
              totalShare.toDec().gt(new Dec(0))
            ) {
