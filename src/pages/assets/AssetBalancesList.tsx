...
import { ButtonFaint, ButtonSecondary } from 'components/layouts/Buttons';
import { Text, TitleText } from 'components/Texts';
import { IBCAssetInfos } from 'config';
import { TransferDialog } from 'dialogs/Transfer';
import { TableData, TableHeaderRow } from 'pages/assets/components/Table';
...
export const AssetBalancesList = observer(function AssetBalancesList() {
  ...
  const ibcBalances = IBCAssetInfos.map((channelInfo) => {
    ...
    let ibcDenom = makeIBCMinimalDenom(
      channelInfo.sourceChannelId,
      channelInfo.coinMinimalDenom
    );
    ...
    if (channelInfo.ibcTransferPathDenom) {
      ibcDenom = makeIBCMinimalDenom(
        channelInfo.sourceChannelId,
        channelInfo.ibcTransferPathDenom
      );
      ...
      <table className="w-full pb-8">
        ...
        <tbody className="w-full">
          ...
          {ibcBalances.map((bal) => {
            ...
            const totalFiatValue = priceStore.calculatePrice(
              bal.balance,
              'usd'
            );
  ...
  return (
    <thead>
      <TableHeaderRow>
        <TableData
          style={{
            width: isMobileView ? tableWidthsOnMobileView[0] : tableWidths[0]
          }}
        >
          ...
          >
            <Img
              ...
              style={{
                width: `2.5rem`,
                height: `2.5rem`,
                marginRight: isMobileView ? 10 : 20
              }}
              ...
              {onDeposit ? (
                <React.Fragment>
                  <div className="relative group">
                    ...
                    >
                      <p className="text-sm text-secondary-200 leading-none">
                        Deposit
                      </p>
                      <img alt="right" src={'/assets/Icons/Right.svg'} />
                    </ButtonFaint>
                    {isUnstable ? (
                      <div
                      ...
                      >
                        <span>
                          IBC deposit/withdrawal is temporarily disabled
                        </span>
                      </div>
                    ) : null}
                  </div>
