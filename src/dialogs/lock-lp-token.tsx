...
import { TableBodyRow, TableData, TableHeadRow } from 'components/Tables';
  ...
  observer(
    ({
      ...
      const lockableDurations =
        queries.osmosis.queryLockableDurations.lockableDurations;
      ...
      const [isValidatorSelectStage, setIsValidatorSelectStage] =
        useState(false);
      const isShowingSuperfluidCheckbox =
        selectedDurationIndex === lockableDurations.length - 1 &&
        isSuperfluidEnabled &&
        hasNotExistSuperfluidLock;
      useEffect(() => {
      ...
      return (
        <div className="text-white-high w-full h-full">
          {!isValidatorSelectStage ? (
            <React.Fragment>
              {' '}
              <h5 className="text-lg md:text-xl mb-5 md:mb-9">
                Bond LP tokens
              </h5>
              ...
              <ul className="flex flex-col gap-2.5 mb-5 md:flex-row md:gap-6 md:mb-6">
                {lockableDurations.map((duration, i) => {
                  ...
                  if (
                    lockableDurations.length - 1 === i &&
                    isSuperfluidEnabled
                  ) {
                    const superfluidAPY =
                      queries.cosmos.queryInflation.inflation.mul(
                        queries.osmosis.querySuperfluidOsmoEquivalent.estimatePoolAPROsmoEquivalentMultiplier(
                          poolId
                        )
                      );
                ...
                <div className="mb-6">
                  ...
                  >
                    ...
                    <div className="w-6 h-6">
                      <Img src={'/assets/Icons/superfluid-osmo.svg'} />
                    </div>
                  </label>
                </div>
              )}
                ...
                >
                  ...
                  account.isSendingMsg === 'superfluidDelegate' ? (
                    ...
                    <p className="text-white-high font-semibold text-base md:text-lg">
                      {isSuperfluidEnabled && isCheckedSuperfluid
                        ? 'Next'
                        : 'Bond'}
                    </p>
                  )}
              ...
              header={
                <React.Fragment>
                  ...
                  <h5 className="ml-9 text-lg md:text-xl">
                    Select{' '}
                    <span className="hidden md:inline">{'Superfluid '}</span>
                    Validator
                  </h5>
                </React.Fragment>
              }
      ...
      return (
        <div className="text-white-high w-full h-full">
          ...
          <LockLpTokenValidatorSelectStageViewInDialog
            ...
            onSumbit={async (selectedValidatorAddress: string) => {
              try {
                await account.osmosis.sendSuperfluidDelegate(
                  lockIds,
                  selectedValidatorAddress,
                  '',
                  () => {
...
export const LockLpTokenValidatorSelectStageViewInDialog: FunctionComponent<{
  ...
  ({
    ...
    const activeValidatorsResult =
      queries.cosmos.queryValidators.getQueryStatus(Staking.BondStatus.Bonded);
    ...
    const [selectedValidatorAddress, setSelectedValidatorAddress] =
      useState('');
            ...
            <thead className="sticky top-0 z-10 bg-surface">
              <TableHeadRow className="!px-5 !py-0 !mt-0 !rounded-t-2xl">
                ...
                <TableData
                  as="th"
                  width={'20%'}
                  className="!px-0 !py-3 justify-end"
                >
              ...
              {delegatedValidators
                ...
                .filter((activeValidator) => {
                  if (searchingValidatorValue) {
                    const matchingValidatorName =
                      activeValidator.description.moniker?.toLowerCase().trim();
                    return matchingValidatorName?.includes(
                      searchingValidatorValue
                    );
                  } else {
                    return true;
                  }
                ...
                .map((activeValidator) => {
                  const validatorThumbnail =
                    activeValidatorsResult.getValidatorThumbnail(
                    ...
                    <TableBodyRow
                      ...
                      onClick={() =>
                        setSelectedValidatorAddress(
                          isSelected ? '' : activeValidator.operator_address
                        )
                      }
                    >
                      <TableData
                      ...
                      >
                        <div className="rounded-full border border-enabledGold mr-3 w-9 h-9 p-0.75 flex justify-center items-center">
                          <img
                            src={
                              validatorThumbnail || '/assets/Icons/Profile.svg'
                            }
        ...
        <div className="flex w-full justify-center items-center">
          <button
            ...
            disabled={
              !account.isReadyToSendMsgs ||
              selectedValidatorAddress === '' ||
              checkAmountConfigError(amountConfig)
            }
            onClick={async (e) => {
              e.preventDefault();
