...
import { IconExternalLink, IconCheckBox } from 'icons';
                  ...
                  .add(
                    new CoinPretty(
                      ...
                      new Dec(config.amount).mul(
                        DecUtils.getPrecisionDec(config.currency.coinDecimals)
                      )
                    ).toDec()
                  )
              )
...
export class RemoveLiquidityConfig extends ManageLiquidityConfigBase {
  ...
  get poolShareWithPercentage(): CoinPretty {
    return this.poolShare.mul(
      new Dec(this.percentage.toString()).quo(DecUtils.getPrecisionDec(2))
    );
  }
}
    ...
    <ul className="w-full h-8 grid grid-cols-2">
      <li
        ...
        className={cn(
          ...
          tab === Tabs.ADD
            ? 'border-b-2'
            : 'border-b border-opacity-30 hover:border-opacity-100'
        )}
      >
        <p
          className={cn(
    ...
    <React.Fragment>
      <p className="text-xs text-white-disabled mb-3 md:mb-4.5">
        ...
        <span className="ml-1 text-secondary-200">
          {poolShare.shrink(true).trim(true).toString()}
        </span>
      </p>
        ...
        {(isMouseOverInfoIcon || isMouseOverToolTip) && (
          ...
          >
            <div className="text-white-high text-sm mb-1 leading-tight">
              Single Asset LP allows you to provide liquidity using one asset.
              However, this will impact the pool price of the asset you’re
              providing liquidity with.
            </div>
          </div>
        )}
      </div>
          ...
          <div className="flex justify-between">
            ...
            <div className="text-sm leading-6 text-white-high">
              {(() => {
                ...
                if (value)
                  return value.maxDecimals(3).trim(true).toString() + '%';
              })()}
            </div>
          </div>
        </div>
...
const TokenLiquidityItem: FunctionComponent<{
  ...
  ({ addLiquidityConfig, index, isSingleAmountIn: isSingleAmountIn }) => {
    const { chainStore, queriesStore } = useStore();
    const { isMobileView } = useWindowSize();
    const [isTokenDropdownOpen, setIsTokenDropdownOpen] =
      useBooleanStateWithWindowEvent(false);
        ...
        <section className="flex items-center justify-between">
            ...
            {isSingleAmountIn ? (
              <TokenSelect
                options={addLiquidityConfig.poolAssets
                  .filter(
                    (poolAsset) =>
                      poolAsset.currency.coinDenom !== currentCurrency.coinDenom
                  )
                ...
                onSelect={(appCurrency: AppCurrency) => {
                  const index = addLiquidityConfig.poolAssets.findIndex(
                    (poolAsset) =>
                      poolAsset.currency.coinDenom === appCurrency.coinDenom
                  );
                ...
                extraAssetInfos={addLiquidityConfig.poolAssets.map(
                  (poolAsset, index) => ({
                    index,
                    coinDenom: poolAsset.currency.coinDenom,
                    liquidityWeightPercentage: poolAsset.weight
                      .quo(addLiquidityConfig.totalWeight)
                      .decreasePrecision(2)
                  })
                )}
              ...
              <>
                <figure
                  ...
                  className={cn(
                    ...
                    `p${percentage.maxDecimals(0).locale(false).toString()}`
                  )}
                >
                  ...
                  <div className="slice">
                    <div
                      style={{
                        background: `${borderImages[MISC.GRADIENTS[index]]}`
                      }}
                      className="bar"
                    />
                    <div className="fill" />
                ...
                <h5 className="text-base md:text-xl">
                  {currentCurrency.coinDenom.toUpperCase()}
                </h5>
              </>
            )}
          </div>
          <div className="flex flex-col items-end ml-6 md:ml-0 relative">
            <div className="flex items-center justify-center md:mb-2">
              <p
                className={`hidden md:block text-xs md:text-sm mb-0.5 md:mb-0`}
              >
                Available{' '}
                  ...
                  try {
                    ...
                    await account.osmosis.sendJoinSwapExternAmountInMsg(
                      addLiquidityConfig.poolId,
                      {
                        ...
                        currency:
                          addLiquidityConfig.singleAmountInConfig.currency
                      },
                      '2.5',
                      '',
                      () => {
