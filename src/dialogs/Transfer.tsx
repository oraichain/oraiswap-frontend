...
export const TransferDialog = wrapBaseDialog(
  observer(
    ({
      ...
      const counterpartyBech32Prefix =
      ...
      const [counterpartyInitAttempted, setCounterpartyInitAttempted] =
        useState(false);
                      ...
                      {(isMouseOverInfoIcon || isMouseOverToolTip) && (
                        ...
                        >
                          <div className="text-white-high text-sm mb-1 leading-tight">
                            Incorrect withdrawal address could result in loss of
                            funds. Avoid withdrawal to exchange deposit address.
                          </div>
                        </div>
                      )}
                    </div>
          ...
          <div className="w-full mt-6 md:mt-9 flex items-center justify-center">
            {!isAccountConnected ? (
                  ...
                  try {
                    if (isWithdraw) {
                      if (
                        account.isReadyToSendMsgs &&
                        (counterpartyAccount.bech32Address ||
                          (customWithdrawAddr && isValidCustomWithdrawAddr))
                      ) {
                                ...
                                for (const event of events) {
                                  if (event.type === 'send_packet') {
                                    ...
                                    const sequenceAttr = attributes.find(
                                      (attr) =>
                                        attr.key ===
                                        Buffer.from('packet_sequence').toString(
                                          'base64'
                                        )
                                    );
                                    const sequence = sequenceAttr
                          ...
                          await counterpartyAccount.cosmwasm.sendExecuteContractMsg(
                            ...
                            currency.originCurrency.contractAddress,
                            {
                              send: {
                                ...
                                msg: Buffer.from(JSON.stringify(msg)).toString(
                                  'base64'
                                )
                              }
                            },
              ...
              >
                ...
                (!isWithdraw &&
                  counterpartyAccount.isSendingMsg === 'ibcTransfer') ? (
                  <svg
