...
export const useCreateNewPoolConfig = (
  ...
  const [config] = useState(
    () => new CreateNewPoolConfig(chainGetter, chainId, sender, queryBalances)
  );
...
export const CreateNewPoolDialog = wrapBaseDialog(
  observer(({ close }: { close: () => void }) => {
    ...
    const feeConfig = useFakeFeeConfig(
      chainStore,
      chainStore.current.chainId,
      account.msgOpts.createPool.gas
    );
    ...
    const content = (() => {
      if (stage === 1) return <NewPoolStage1 config={config} close={close} />;
        ...
        return <NewPoolStage2 config={config} close={close} />;
      else if (stage === 3)
        return <NewPoolStage3 config={config} close={close} />;
    })();
    return (
...
const NewPoolButton: FunctionComponent<{
  ...
  const [isPoolCreationFeeChecked, setIsPoolCreationFeeChecked] =
    useState(false);
  ...
  return (
    <React.Fragment>
      ...
      {stage === 3 ? (
        <div className="flex flex-row justify-center items-center mt-5">
          ...
          >{`I understand that creating a new pool will cost ${queryPoolCreationFee.poolCreationFee
            .map((fee) => fee.trim(true).maxDecimals(6).toString())
            .join(',')}`}</p>
        </div>
      ) : null}
      <div className="flex items-center justify-center w-full">
        <div
          className={cn(
            'mt-7.5 h-15 gap-4 flex items-center justify-center',
            stage > 1 ? 'w-4/5' : 'w-full'
          )}
        >
          {stage > 1 && (
            <button
