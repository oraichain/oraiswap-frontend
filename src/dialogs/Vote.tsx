...
export const VoteDialog = wrapBaseDialog(
  ({ proposalIndex, close }: { proposalIndex: number; close: () => void }) => {
      ...
      <div className="text-white-high w-full h-full">
        <div className="w-full flex items-center justify-between mb-9">
          ...
          <button onClick={close} className="hover:opacity-75 cursor-pointer">
            <Img className="w-6 h-6" src={'/assets/Icons/X.svg'} />
          </button>
        </div>
        ...
        <ul className="mt-10">
          <li className="h-15 border-b border-white-faint flex items-center">
            ...
            >
              <figure
                className={cn('rounded-full w-4 h-4 bg-secondary-200', {
                  hidden: vote !== Vote.Yes
                })}
              />
            </figure>
            <h6>Yes</h6>
