...
import { osmosis } from 'libs/proto/osmosis';
  ...
  async sendJoinSwapExternAmountInMsg(
    ...
    await this.base.sendMsgs(
      ...
      async () => {
        ...
        const estimated = pool.estimateJoinSwapExternAmountIn(
          tokenIn,
          this.base.msgOpts.joinPool.shareCoinDecimals
        );
    ...
    await this.base.sendMsgs(
      ...
      async () => {
        ...
        for (const route of routes) {
          const queryPool =
            queries.osmosis.queryGammPools.getObservableQueryPool(route.poolId);
          await queryPool.waitFreshResponse();
        ...
        if (tx.code == null || tx.code === 0) {
          ...
          for (const route of routes) {
            queries.osmosis.queryGammPools
              .getObservableQueryPool(route.poolId)
              .fetch();
          }
        }
  ...
  async sendExitPoolMsg(
    ...
    await this.base.sendMsgs(
      ...
      async () => {
        ...
        const estimated = pool.estimateExitSwap(
          shareInAmount,
  ...
  async sendBeginUnlockingMsgOrSuperfluidUnbondLockMsgIfSyntheticLock(
    ...
    const protoMsgs = msgs.map((msg) => {
      if (msg.type === this.base.msgOpts.beginUnlocking.type && msg.value.ID) {
        ...
        msg.type === this.base.msgOpts.superfluidUndelegate.type &&
        msg.value.lock_id
      ) {
        ...
        msg.type === this.base.msgOpts.superfluidUnbondLock.type &&
        msg.value.lock_id
      ) {
